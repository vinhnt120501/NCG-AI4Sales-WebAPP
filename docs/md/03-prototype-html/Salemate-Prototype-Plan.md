...
> Mục tiêu prototype: **bản trình bày để pitch Anova Feed**. Ưu tiên trải nghiệm "wow", chạy được vài kịch bản thật, đánh trúng pain point. KHÔNG cần độ chính xác/độ phủ production.
> Phạm vi: làm **cả 4 tính năng** nhưng theo thứ tự ưu tiên + 1 "golden path" trình diễn xuyên suốt.
...
**Nguyên tắc xuyên suốt:** mỗi tính năng khi trình bày phải kết thúc bằng "câu chốt giúp Sales" (luận điểm bán hàng, phác đồ, kịch bản) — không chỉ trả lời thông tin.

---

## 1. Kiến trúc tổng thể (đủ cho bản trình bày, không over-engineer)
...
- **Cách làm:** trích thành JSON, mỗi SKU 1 bản ghi. Trường gợi ý:
  ...
  ```
- Với bản trình bày: chuẩn hóa **20–30 SP chủ lực** là đủ (phần còn lại điền tối thiểu). Slide cũng đề xuất giai đoạn 1 phạm vi hẹp.
...
- **Cách làm:** chunk theo chủ đề; **gắn cờ `r&d_approved: true/false`** cho từng tài liệu. Bản trình bày chỉ cho bot dùng tri thức `approved` → thể hiện "tri thức được R&D kiểm duyệt".
...
### 2.4 Dữ liệu đối thủ — *web search + sinh dữ liệu minh họa bằng Claude*
...
- **Chỗ thiếu:** dùng Claude **sinh dữ liệu mẫu hợp lý** (thành phần dinh dưỡng, giá ước tính, FCR) → **gắn nhãn rõ "DỮ LIỆU MINH HỌA"** trong UI và file, tránh hiểu nhầm là số liệu thật.
...
> Mẹo trình bày: chuẩn bị sẵn **bộ 40–50 câu hỏi mẫu** + đáp án kỳ vọng. Vừa để test, vừa để show "độ chính xác X%" lúc pitch.
...
- **Output:** trình diễn được "hỏi sản phẩm/chính sách → trả lời nhanh, dẫn nguồn".
...
- **Output:** trình diễn được "khách đang dùng cám X → bảng so sánh + kịch bản chào bán".
...
- **Output:** trình diễn được pain sâu nhất một cách an toàn.

### Phase 4 — UI polish + kịch bản trình diễn + tổng duyệt (2–3 ngày)
- Style giao diện theo brand FPT/Anova; thêm badge "nguồn", "minh họa", "confidence".
- Hard-code/tinh chỉnh 4 kịch bản golden path; tổng duyệt, đo thời gian phản hồi.

---

## 4. Kịch bản trình diễn (Golden Path) — bán câu chuyện, không bán tính năng
...
- [ ] UI chat + badge nguồn/minh họa/confidence
...
UI hiển thị realtime các tool được gọi (khối "Salemate đang xử lý...") để minh bạch khi trình bày.
...
Số liệu là **dữ liệu minh họa (cook)**; production thay bằng dữ liệu thật (giữ nguyên schema → không phải sửa code).
...
- **Local/trình bày:** `streamlit run app.py` (đã có sẵn `.venv`).
