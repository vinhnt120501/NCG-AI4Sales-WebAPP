...
Mỗi câu trả lời kết bằng dòng **"👉 Gợi ý cho Sales"** (luận điểm chốt đơn). Định dạng: chat tiếng Việt có cấu trúc, dẫn nguồn tài liệu, gắn nhãn **DEMO**, cảnh báo an toàn.
...
|---|---|---|---|
| Catalogue thuốc/vaccine | `data/vet_products.json` | Cook bằng AI, bám thương hiệu thật (Anova Pharma, **Navetco/NAVET-ASFVAC**, Vetvaco, Anova Biotech) — web-search xác thực cấu trúc nhóm cty + vaccine ASF VN | Cấu trúc thật, **số liệu liều lượng/giá là DEMO** |
| KB bệnh | `data/diseases.json` | Cook từ kiến thức thú y phổ thông + web-search ASF 2025 (NAVET-ASFVAC, AVAC, diễn biến dịch) | DEMO, định hướng đúng |
| Thông tin thị trường | (trong `diseases.json`) | Cook từ tin tức ASF 2025 | DEMO |
| Nhà phân phối + lịch sử đơn | `data/distributors.json` | Cook hoàn toàn (giả lập) | DEMO |
| Tài liệu RAG | `data/knowledge/*.md` | Cook (an toàn sinh học/ASF, vaccine, chính sách, khuyến mãi) | DEMO |
...
**Tóm lại:** kiến trúc/luồng xử lý là **thật và chạy được**; dữ liệu nghiệp vụ là **mẫu (cook)** để demo. Lên production chỉ cần thay bằng catalogue/KB/CRM thật (giữ nguyên schema).
...
2. **Dữ liệu thật** — đang là DEMO cook; cần nối catalogue/KB thật + CRM/ERP cho lịch sử NPP.
